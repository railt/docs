...
Для получения списка всех переменных, переданных в запросе существует метод 
`getVariables`, возвращающий массив, вида `имя => значение`.
...
- `isQuery` - метод, возвращает `true` в том случае, если основной запрос является 
выборкой (`query`).
- `isMutation` - метод, возвращает `true` в том случае, если основной запрос является 
обновлением (`mutation`).
- `isSubscription` - метод, возвращает `true` в том случае, если основной запрос является 
подпиской (`subscription`).
